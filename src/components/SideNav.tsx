...
const SideNav = () => {
  ...
  return (
    <div className="border-r border-gray-700 w-64 flex flex-col h-full bg-gradient-to-b bg-zinc-800/30 z-20 gap-y-4">
      ...
      {!user.isSignedIn ? (
        <div
          className={`flex flex-row items-center justify-center rounded-md bg-gradient-to-r from-blue-400 to-blue-600 w-24 h-10
        ...
        <>
          <div className="hidden md:flex flex-col items-center justify-end my-4 gap-4">
            <Link
              className="flex flex-row items-center-center gap-2 text-lg py-1 px-2 rounded-md hover:scale-105 hover:shadow-lg hover:bg-zinc-800/50 transition duration-150 ease-in-out"
              href={"/app/settings/profile"}
            >
              <h2 className="my-auto">{user.user.fullName}</h2>
